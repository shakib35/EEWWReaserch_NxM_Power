...
nRIS_rate_case1 = zeros(1, length(pow_vec));
nRIS_rate_case2 = zeros(1, length(pow_vec));
nRIS_rate_case3 = zeros(1, length(pow_vec));

disp('***Alternating Optimization Started***');
disp('--------------------------');
Sum_idx = 1:10;

% Use a parallel for loop to run iterations concurrently
parfor idx_npow = 1:length(pow_vec)
    temp_total_rate_case1 = zeros(1, length(Sum_idx));
    temp_total_rate_case2 = zeros(1, length(Sum_idx));
    temp_total_rate_case3 = zeros(1, length(Sum_idx));
    ...
    for idx = 1:length(Sum_idx)
        % Channel generation code goes here
        % NLoS link: BS1 to EU
        ...
        temp_total_rate_case1(idx) = SU1_rate_case1 + SU2_rate_case1 + EU_rate_case1;
        ...
        % Collecting the results
        temp_total_rate_case2(idx) = SU1_rate_case2 + SU2_rate_case2 + EU_rate_case2;
        ...
        % Collecting the results
        temp_total_rate_case3(idx) = SU1_rate_case3 + SU2_rate_case3 + EU_rate_case3;
    end

    % Compute the mean total rates
    nRIS_rate_case1(idx_npow) = mean(temp_total_rate_case1);
    nRIS_rate_case2(idx_npow) = mean(temp_total_rate_case2);
    nRIS_rate_case3(idx_npow) = mean(temp_total_rate_case3);
end
...
for idx_npow = 1:length(pow_vec)
    ...
    fprintf(fileID_case1_vs_pow, '%7d %10.2f\n', pow_vec(idx_npow), nRIS_rate_case1(idx_npow));
    fprintf(fileID_case2_vs_pow, '%7d %10.2f\n', pow_vec(idx_npow), nRIS_rate_case2(idx_npow));
    fprintf(fileID_case3_vs_pow, '%7d %10.2f\n', pow_vec(idx_npow), nRIS_rate_case3(idx_npow));

    fclose(fileID_case1_vs_pow);
    fclose(fileID_case2_vs_pow);
    fclose(fileID_case3_vs_pow);
    fclose(fileID_p_case1);
    fclose(fileID_v_case1);
    fclose(fileID_p_case2);
    fclose(fileID_v_case2);
    fclose(fileID_p_case3);
    fclose(fileID_v_case3);
end
