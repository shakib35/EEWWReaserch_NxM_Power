function [sumrate, SU1_rate, SU2_rate, EU_rate,pow_vec,v]=AO_CaseThree_cvx(vec_theta, w, r_min_EU, PS1, PS2, PB1, PB2,...
...
tol = 1e-4; 
 while(1)     
    ...
    for iIteration=1:nIterations % for AO algorithm 
        [w, pow_vec, cvx_status] = iterative_beamform_and_power_opti_case3_cvx(vec_theta, r_min_EU, PS1, PS2, PB1, PB2,...
                ch_BS1_to_EU1, ch_BS2_to_EU1, ch_RIS3_to_EU, ch_BS1_to_RIS3, ch_BS2_to_RIS3, ch_SU1_to_EU1, ...
                ch_SU2_to_EU1, ch_BS1_to_SU1,  ch_BS2_to_SU2, ch_RIS3_to_SU1, ch_SU1_to_RIS3, ch_RIS3_to_SU2, ch_SU2_to_RIS3, noise_power, nIterations);                      
        ...
        [vec_theta, pow_vec, cvx_status] = iterative_phase_and_power_opti_case3_cvx(w, r_min_EU, PS1, PS2, PB1, PB2,...
                ch_BS1_to_EU1, ch_BS2_to_EU1, ch_RIS3_to_EU, ch_BS1_to_RIS3, ch_BS2_to_RIS3, ch_SU1_to_EU1, ...
                ch_SU2_to_EU1, ch_BS1_to_SU1,  ch_BS2_to_SU2, ch_RIS3_to_SU1, ch_SU1_to_RIS3, ch_RIS3_to_SU2, ch_SU2_to_RIS3, noise_power, nIterations);         
...
% compute the Edge user's rate for each iteration
        ...
        EU_rate = log2(1+ (norm(ch_BS1_to_EU1'*w(:,3) + v'*Theta_mat_BS1_RIS_EU*w(:,3))^2 + ...
            norm(ch_BS2_to_EU1'*w(:,4)+ v'*Theta_mat_BS2_RIS_EU*w(:,4))^2)/(norm(ch_BS1_to_EU1'*w(:,1) + v'*Theta_mat_BS1_RIS_EU*w(:,1))^2 +...
            norm(ch_BS2_to_EU1'*w(:,2)+ v'*Theta_mat_BS2_RIS_EU*w(:,2))^2 + noise_power) + (pow_vec(1)*(norm(ch_SU1_to_EU1)^2 + ...
            norm(v'*theta_vec_SU1_RIS_EU)^2) + pow_vec(2)*(norm(ch_SU2_to_EU1)^2 + norm(v'*theta_vec_SU2_RIS_EU)^2))/noise_power);
