function [w, p, status_cvx] = iterative_beamform_and_power_opti_cvx(vec_theta, r_min, PS1, PS2, PB1, PB2,...
...
pt_next = 0.05*ones(7,1);
qt_next = 0.05*ones(7,1);
% u_next = 0.5*ones(1,1);
...
while(1)
    ...
    for iIteration=1:nIterations
        cvx_begin quiet  
        variables   t(2) z eta(3) p(2) u b(2) pt(7) qt(7);
        ...
        % UE1  Eq. 22e      
        norm([sqrt(noise_power); ch_BS1_to_SU1'*w(:,1) + v'*Theta_mat_BS1_RIS_SU1*w(:,1)]) <= sqrt(eta(1)); % Eq. 29b
%         
%         % UE2 Eq. 22g
        norm([sqrt(noise_power); ch_BS2_to_SU2'*w(:,2)]) <= sqrt(eta(2)); % Eq. 29c
        
        % Edge UE Eq. 22j 
        norm([sqrt(noise_power); ch_BS2_to_EU'*w(:,2); ch_BS1_to_EU'*w(:,1) + v'*Theta_mat_BS1_RIS_EU*w(:,1)]) <= sqrt(eta(3));
        
%         norm(v'*theta_vec_SU1_RIS_EU) <= sqrt((noise_power*(u_next - u) + norm(ch_SU2_to_EU)^2*(p_next(2) - p(2)))/p_next(1)...
%             - (noise_power*r_min_EU - noise_power*u_next - norm(ch_SU1_to_EU)^2 - p_next(2)*norm(ch_SU2_to_EU)^2)*(p(1) - p_next(1))/p_next(1)^2);

        % Eq. 22h
        pt(6) == real(v'*theta_vec_SU1_RIS_EU);
        qt(6) == imag(v'*theta_vec_SU1_RIS_EU);
        p(1)*(pt_next(6)^2 + qt_next(6)^2) + 2*p_next(1)*pt_next(6)*(pt(6) - pt_next(6))...
           +2*p_next(1)*qt_next(6)*(qt(6) - qt_next(6)) >= noise_power*r_min_EU - noise_power*u - p(1)*norm(ch_SU1_to_EU)^2  - p(2)*norm(ch_SU2_to_EU)^2 ;
...
%         pt_next(1)*(2*pt(1)-pt_next(1)) + qt_next(1)*(2*qt(1) - qt_next(1)) >= t(1)*noise_power;
        ...
        % Eq. 22c 
        pt(2) == (real(ch_BS2_to_SU2'*w(:,2)));
        qt(2) == (imag(ch_BS2_to_SU2'*w(:,2)));
...
% %        
        % Eq. 22f
        pt(4) == real(ch_BS2_to_SU2'*w(:,4));
        qt(4) == imag(ch_BS2_to_SU2'*w(:,4));
...
% %         
        ...
        pt(7) ==real(ch_BS2_to_EU'*w(:,4));
        qt(7) ==imag(ch_BS2_to_EU'*w(:,4));
...
%             % compute the Edge user's rate for each iteration
            ...
            EU_rate = log2(1+ norm(ch_BS1_to_EU'*w(:,3) + v'*Theta_mat_BS1_RIS_EU*w(:,3))^2/(norm(ch_BS1_to_EU'*w(:,1) + v'*Theta_mat_BS1_RIS_EU*w(:,1))^2 + noise_power) + (p(1)*(norm(ch_SU1_to_EU)^2 + norm(v'*theta_vec_SU1_RIS_EU)^2) + p(2)*norm(ch_SU2_to_EU)^2)/noise_power);
...
%                 keyboard;
                ...
                status_cvx = 0;
                fprintf('BEAMFORM-OPTI(CASE-1):%s\n', cvx_status');
                return;
            end
        else
            fprintf('BEAMFORM-OPTI(CASE-1):%s\n', cvx_status');
