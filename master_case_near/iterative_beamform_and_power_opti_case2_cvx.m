function [w, p, status_cvx] = iterative_beamform_and_power_opti_case2_cvx(vec_theta, r_min, PS1, PS2, PB1, PB2,...
...
eta_next = 0.05*ones(3,1);
pt_next = 0.5*ones(8,1);
...
while(1)
    ...
    for iIteration=1:nIterations
        ...
        % Eq. 43h
        pt(6) == real(v(:,1)'*theta_vec_SU1_RIS_EU);
        qt(6) == imag(v(:,1)'*theta_vec_SU1_RIS_EU);
...
%         pt_next(2)*(2*pt(2)-pt_next(2)) + qt_next(2)*(2*qt(2) - qt_next(2)) >= t(2)*noise_power;
        ...
        pt(7) == real(ch_BS2_to_EU'*w(:,4) + v(:,2)'*Theta_mat_BS2_RIS_EU*w(:,4));
        qt(7) == imag(ch_BS2_to_EU'*w(:,4) + v(:,2)'*Theta_mat_BS2_RIS_EU*w(:,4));
...
%                 keyboard;
                ...
                status_cvx = 0;
                fprintf('BEAMFORM-OPTI(CASE-2):%s\n', cvx_status');
                return;
            end
        else
%             keyboard;
